...
def run_model(
    ...
    s_screen_interval=None,
    t_screen_interval=None,
    student_mask=False,
    teacher_mask=False,
    half_classes=False,
    ventilation_mod=1,
    ...
    model = SEIRX_school(
        ...
        infection_risk_contact_type_weights=simulation_params[
        ...
        follow_up_testing_interval=measures["follow_up_testing_interval"],
        liberating_testing=measures["liberating_testing"],
        index_case=index_case,
        agent_types=agent_types,
        age_transmission_risk_discount=simulation_params["age_transmission_discount"],
        ...
        transmission_risk_vaccination_modifier=measures["vaccination_modification"],
        seed=seed,
    )
        ...
        if (
            len(
                [
                    ...
                    if (a.exposed == True or a.infectious == True)
                ]
            )
            == 0
        ):
...
def run_ensemble(
    ...
    agent_types["family_member"]["vaccination_ratio"] = family_member_vaccination_ratio

    sname = "{}_classes-{}_students-{}".format(
        school_type, characteristics["classes"], characteristics["students"]
    )
    ...
    measure_string = "{}_test-{}_turnover-{}_index-{}_tf-{}_sf-{}_tmask-{}".format(
        school_type,
        test,
        turnover,
        index_case[0],
        ...
        s_screen_interval,
        bmap[teacher_mask],
    ) + "_smask-{}_half-{}_vent-{}_tvacc-{}_svacc-{}".format(
        bmap[student_mask],
        bmap[half_classes],
        ventilation_mod,
...
def get_data(stype, src_path):
    ...
    for f in files:
        screening_params, agents, half = af.get_measures(
            f.strip(".csv"), vaccinations=True
        )
...
def set_individual_measures(data):
    ...
    data.loc[
        data[
            (data["transmission_risk_ventilation_modifier"] == 0.36)
            & (data["student_mask"] == False)
            & (data["teacher_mask"] == False)
            & (data["student_screening_interval"] == "never")
            & (data["teacher_screening_interval"] == "never")
...
def set_measure_packages(data):
    ...
    # ventilation + halved classes + masks teachers + masks students + tests teachers 2x + tests students 2x
    data.loc[
        data[
            (data["transmission_risk_ventilation_modifier"] == 0.36)
            & (data["student_mask"] == True)
        ...
        "measure",
    ] = "ventilation + halved classes + masks teachers & students + tests 2x teachers & students"

    # ventilation + halved classes + tests teachers 2x + tests students 2x
    data.loc[
        data[
            (data["transmission_risk_ventilation_modifier"] == 0.36)
            & (data["student_mask"] == False)
        ...
        "measure",
    ] = "ventilation + halved classes + tests 2x teachers & students"
